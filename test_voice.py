...
def test_voice_to_voice():
    ...
    print("\n[1/4] AUDIO INPUT")
    choice = input("Record new audio? (y/n): ").lower().strip()

    if choice == "y":
        duration_raw = input("Recording duration in seconds (default 30): ").strip()
    ...
    if not text_query:
        print("No query text available. Exiting.")
        return
...
def test_asr_only():
    ...
    choice = input("Record audio? (y/n): ").lower().strip()
    if choice == "y":
